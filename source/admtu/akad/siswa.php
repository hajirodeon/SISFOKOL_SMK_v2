	...
	if ((empty($nis)) OR (empty($nama)))
		...
		if (empty($s))
			...
			if ($tcc > 1)
				...
				//insert ke kelas
				mysql_query("INSERT INTO siswa_kelas(kd, kd_tapel, kd_keahlian, kd_kelas, kd_siswa) VALUES ".
								"('$xx', '$tapelkd', '$keakd', '$kelkd', '$x')");
...
if (empty($tapelkd))
	...
	if ($count != 0)
		...
		do
			...
			<td>
			<a href="'.$filenya.'?s=edit&tapelkd='.$tapelkd.'&keakd='.$keakd.'&kelkd='.$kelkd.'&swkd='.$kd.'"><img src="'.$sumber.'/img/edit.gif" width="16" height="16" border="0"></a>
			</td>
			<td valign="top">
			'.$nis.'
			</td>
