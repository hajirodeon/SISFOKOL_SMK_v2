...
for ($we=0;$we<=($npage);$we++)
	...
	//mapel
	$qpel = mysql_query("SELECT DISTINCT(m_prog_pddkn.xpel) ".
	...
	do
		...
		//deteksi, jika lebih dari 15 karakter
		if (strlen($pel) >= 15)
			{
			$pel1 = substr($pel,0,15);
			$pel = "$pel1...";
			}
	...
	do
		...
		do
			...
			//mapel e....
			$qtu = mysql_query("SELECT * FROM m_prog_pddkn ".
									"WHERE xpel = '$xpel'");
			...
			$qrt = mysql_query("SELECT AVG(ulangan_rata.nilai) AS rtnil ".
									...
									"AND ulangan_rata.kd_prog_pddkn = '$tukd'");
