...
if (empty($tapelkd))
	...
	$qjw = mysql_query("SELECT jadwal.*, jadwal.kd AS jkd, m_guru_prog_pddkn.*, m_guru.*, m_prog_pddkn.*, ".
							"m_keahlian.*, m_kelas.*, m_ruang.*, m_hari.*, m_jam.* ".
							...
							"ORDER BY m_keahlian.bidang, m_kelas.no, m_ruang.ruang, ".
	...
	if ($tjw == 0)
		...
		do
			...
			$nomer = $nomer + 1;
			$jw_kd = nosql($rjw['jkd']);
			$jw_bid = balikin($rjw['bidang']);
			$jw_prog = balikin($rjw['program']);
