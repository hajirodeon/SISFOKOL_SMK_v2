...
if ($HTTP_POST_VARS['btnRST'])
	...
	if ($tpkd == "tp01")
		...
		$ke = "$filenya?tpkd=$tpkd&tipe=$tipe&tapelkd=$tapelkd&keakd=$keakd&kelkd=$kelkd&page=$page";
...
if ($tpkd == "tp01")
	...
	do
		...
		echo '<option value="'.$filenya.'?tpkd='.$tpkd.'&tipe='.$tipe.'&tapelkd='.$tapelkd.'&keakd='.$kea_kd.'">'.$kea_bid.' - '.$kea_pro.'</option>';
		}
	...
	do
		...
		echo '<option value="'.$filenya.'?tpkd='.$tpkd.'&tipe='.$tipe.'&tapelkd='.$tapelkd.'&keakd='.$keakd.'&kelkd='.$bt_kd.'">'.$bt_kelas.'</option>';
		}
